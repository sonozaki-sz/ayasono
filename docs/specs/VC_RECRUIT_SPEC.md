...
最終更新: 2026年3月14日（VC名変更ボタン追加・全ボタン権限を MANAGE_CHANNELS に統一・section 7「VC名の変更」追加）
...
| 管理ボタン | 「🎤 VCに参加」「✏️ VC名を変更」「🔇 VCを終了」「🗑️ 募集を削除」を募集メッセージに設置 |
...
| フィールド | ステップ | 種別                 | 必須 | 制約                                                                  |
| ---------- | -------- | -------------------- | ---- | --------------------------------------------------------------------- |
| メンション | 1        | セレクトメニュー     | ❌   | 設定済みロール一覧 ＋「なし」。未設定時はメンションなし               |
| VC         | 1        | セレクトメニュー     | ✅   | 「🆕 新規VC作成」＋セットアップしたカテゴリーの既存VC一覧             |
| 募集内容   | 2        | テキスト入力（段落） | ✅   | 最大200文字                                                           |
...
<tr><td><kbd>🎤 VCに参加</kbd>（リンクボタン）　<kbd>✏️ VC名を変更</kbd>　<kbd>🔇 VCを終了</kbd>　<kbd>🗑️ 募集を削除</kbd></td></tr>
</table>

> 既存VC選択時・新規VC作成時ともに表示形式は同一です。VC欄の `<#channelId>` は選択または作成したVCのメンションになります。「VCに参加」ボタンは `ButtonStyle.Link` で URL は `https://discord.com/channels/{guildId}/{channelId}` を設定します。「✏️ VC名を変更」「🔇 VCを終了」「🗑️ 募集を削除」ボタンのカスタムIDには投稿者IDとVCチャンネルIDを埋め込みます。VCを終了した後は「VCに参加」「✏️ VC名を変更」「🔇 VCを終了」ボタンが除去され、「🔇 VC終了済み」（無効）に置き換わります。
...
4. 設定されたメンション（ある場合）を付与して 募集一覧 に募集メッセージを送信
   （メッセージには「🎤 VCに参加」「✏️ VC名を変更」「🔇 VCを終了」「🗑️ 募集を削除」ボタンを添付）
   ↓
...
7. 設定されたメンション（ある場合）を付与して 募集一覧 に募集メッセージを送信
   （VCリンクは作成した新規VCのリンク。メッセージには「🎤 VCに参加」「✏️ VC名を変更」「🔇 VCを終了」「🗑️ 募集を削除」ボタンを添付）
   ↓
...
| 条件                                       | VCの扱い                                         | 投稿パネルの更新                                                       |
| ------------------------------------------ | ------------------------------------------------ | ---------------------------------------------------------------------- |
| VCが `createdVoiceChannelIds` に含まれる   | DB から該当IDを削除 → VCを削除                   | 「VCに参加」「✏️ VC名を変更」「🔇 VCを終了」→「🔇 VC終了済み」（無効） |
| VCが `createdVoiceChannelIds` に含まれない | VCは削除しない（既存VCは他メンバーも利用のため） | 「VCに参加」「✏️ VC名を変更」「🔇 VCを終了」→「🔇 VC終了済み」（無効） |
...
4a. 「終了する」押下
   - VCが createdVoiceChannelIds に含まれる場合:
     DB から該当IDを先に削除（channelDelete ハンドラーのスキップを保証） → VCを削除
   - 募集メッセージのボタン行を更新:
     「VCに参加」「✏️ VC名を変更」「🔇 VCを終了」→ 「🔇 VC終了済み」（ButtonStyle.Secondary, disabled）
   - エフェメラルメッセージを「✅ VCを終了しました」に更新
   ↓
4b. 「キャンセル」押下
   → エフェメラルメッセージを「キャンセルしました」に更新して終了
...
> 「🔇 VC終了済み」ボタンは `ButtonStyle.Secondary` かつ `disabled: true` で表示します。「🗑️ 募集を削除」ボタンは終了後も有効なまま残します。「✏️ VC名を変更」ボタンは終了後に除去されます。
...
**動作**: モーダルでVC名を入力し、対象VCのチャンネル名を更新する。VCを終了した後はボタンが除去されるため操作不可。
...
| `createdVoiceChannelIds` | string[]                    | `[]`       | 「新規VC作成」で作成したVCのID一覧。「🔇 VCを終了」または「🗑️ 募集を削除」ボタンで明示的に削除される                     |
...
| VCを終了ボタンを権限のないユーザーが押した       | `❌ この操作は投稿者または管理者のみ実行できます` をエフェメラルで表示                                       |
| VC名変更ボタンを権限のないユーザーが押した       | `❌ この操作は投稿者または管理者のみ実行できます` をエフェメラルで表示                                       |
| 募集投稿後に新規作成VCが手動削除された           | `channelDelete` イベントで `createdVoiceChannelIds` から削除し、投稿を「VC終了済み」状態に更新（詳細は後述） |
| 募集投稿後に既存VCが手動削除された               | 「🔇 VCを終了」ボタン押下時にVCが存在しない場合、VCなし状態として「VC終了済み」扱いで投稿を更新する          |
...
3. 該当メッセージのボタン行を「VC終了済み」状態に更新（「🔇 VCを終了」→「🔇 VC終了済み」（無効）、「VCに参加」「✏️ VC名を変更」ボタンを除去）

> 既存VCが手動削除された場合はチャンネルIDをカスタムIDに持たないため自動更新は行わず、「🔇 VCを終了」ボタン押下時にVCの存在確認を行い「VC終了済み」扱いで更新します。
...
| 操作 | 種別 | 説明 |
| ---- | ---- | ---- |
| メンション選択 | セレクトメニュー | メンション候補ロールを選択（なし含む） |
...
| VCを終了 | ボタン | 権限チェック後に確認プロンプトを表示 |
| VCを終了（確認） | ボタン×2 | 「終了する」「キャンセル」 |
...
| 新規VCの削除                 | 「🔇 VCを終了」または「🗑️ 募集を削除」ボタンによる明示的な削除のみ（自動削除なし）                                            |

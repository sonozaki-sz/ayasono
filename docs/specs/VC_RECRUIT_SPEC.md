...
flowchart TD
    ...
    B -- はい --> D["セレクトメニュー表示（ephemeral）\n設定済みカテゴリーを複数選択可能"]
    D --> E["確認パネル表示（ephemeral）\n選択カテゴリー一覧・注意文・ボタン"]
    E -- 撤去する --> F[撤去処理]
...
<table>
  ...
  <tr><td><button>🗑️ 撤去する</button></td><td><button>キャンセル</button></td></tr>
</table>
...
1. パネルチャンネルのピン留めメッセージ（`panelMessageId`）を削除（存在する場合）
2. パネルチャンネルを削除（既に削除済みの場合はスキップ）
3. 投稿チャンネルを削除（既に削除済みの場合はスキップ）
4. DB のセットアップレコードを削除

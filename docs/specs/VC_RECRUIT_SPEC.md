...
3. モーダル表示（VC名入力フィールドなし）
   - 募集内容を入力して送信
   ↓
4. 設定されたメンション（ある場合）を付与して vc-recruit-board に募集メッセージを送信
   ↓
5. 募集メッセージにスレッドを自動作成（アーカイブ時間: 設定値）
   ↓
6. 送信者を選択したVCへ自動移動
...
4. セットアップカテゴリーに新規VCを作成
   ...
   - 人数制限: なし
     ↓
5. 作成したVCのテキストチャット（チャット）に設定パネルを送信
   ↓
6. 設定されたメンション（ある場合）を付与して vc-recruit-board に募集メッセージを送信
   （VCリンクは作成した新規VCのリンク）
   ↓
   6a. 募集メッセージにスレッドを自動作成（アーカイブ時間: 設定値）
   ↓
7. 作成したVCのIDを DB に保存（createdVoiceChannelIds に追加）
   ↓
8. 送信者を新規作成VCへ自動移動
   ↓
9. エフェメラルメッセージを削除
...
- 注意文: `「選択したカテゴリーのパネルチャンネル・投稿チャンネルが削除されます。この操作は取り消せません。」`
...
<table>
  ...
  <tr><td colspan="2">⚠️ 選択したカテゴリーのパネルチャンネル・投稿チャンネルが削除されます。この操作は取り消せません。</td></tr>
...
2. パネルチャンネルのパネルメッセージを削除（存在する場合）
3. パネルチャンネルを削除（既に削除済みの場合はスキップ）
4. 投稿チャンネルを削除（既に削除済みの場合はスキップ）
...
### パネル/投稿チャンネル削除時（channelDelete イベント）

パネルチャンネルまたは投稿チャンネルが手動削除されると、`channelDelete` イベントハンドラーがペアのチャンネルも削除し、DB のセットアップレコードを自動クリーンアップする。

| 削除されたチャンネル | 動作                                             |
| -------------------- | ------------------------------------------------ |
| パネルチャンネル     | 投稿チャンネルを削除 → DB のセットアップを削除   |
| 投稿チャンネル       | パネルチャンネルを削除 → DB のセットアップを削除 |
...
async function buildVcSelectOptions(
  setup: VcRecruitSetup,
  guild: Guild,
  guildId: string,
): Promise<StringSelectMenuOptionBuilder[]> {
  // VAC のトリガーチャンネルIDを取得し、除外リストを構築する
  const vacConfig = await getBotVacRepository().getVacConfigOrDefault(guildId);
...
> `buildVcSelectOptions` は VAC リポジトリを参照するため `async` 関数になります。
> 呼び出し元（`modalHandler.ts`）も `await` で受け取るよう実装してください。

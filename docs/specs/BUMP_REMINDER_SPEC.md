...
<tr><td><kbd>🔔 通知を受け取る</kbd> &nbsp; <kbd>🔕 通知を受け取らない</kbd></td></tr>
</table>

**機能:**

- `🔔 通知を受け取る`: ユーザーをBump通知リストに登録
- `🔕 通知を受け取らない`: ユーザーをBump通知リストから解除
...
- ユーザー個別のBump通知ON/OFFボタン（パネルUI）実装済み
- Bump検知時にパネルUIが表示され、ユーザーが自分でBump通知の受信を管理可能
...
| `panelMessageId`| String?  | パネルメッセージID（リマインダー送信後に削除）     |

...
- 「サーバー非在籍のみ選択」ボタンでサーバーを退出済みのユーザーのみを選択状態にできる
...
| コンポーネント | emoji | ラベル | スタイル | 動作 |
| --- | --- | --- | --- | --- |
| `bump-reminder:remove-mention-users:first` | ⏮ | ― | Secondary | 最初のページ（1ページ目は `disabled`） |
| `bump-reminder:remove-mention-users:prev` | ◀ | ― | Secondary | 前のページ（1ページ目は `disabled`） |
| `bump-reminder:remove-mention-users:jump` | ― | {{page}}/{{total}}ページ | Secondary | 押下でモーダル表示、番号入力でページジャンプ |
| `bump-reminder:remove-mention-users:next` | ▶ | ― | Secondary | 次のページ（最終ページは `disabled`） |
| `bump-reminder:remove-mention-users:last` | ⏭ | ― | Secondary | 最後のページ（最終ページは `disabled`） |

**Row 2 - セレクトメニュー:**

| コンポーネント | 種別 | 設定 |
| --- | --- | --- |
| `bump-reminder:remove-mention-users:select` | StringSelect | `minValues: 0`, `maxValues: 現ページのユーザー数` |
...
| コンポーネント | emoji | ラベル | スタイル | 動作 |
| --- | --- | --- | --- | --- |
| `bump-reminder:remove-mention-users:select-all` | ☑️ | 全員を選択 | Secondary | 全ページの全ユーザーを選択 |
| `bump-reminder:remove-mention-users:select-left` | 👋 | サーバー非在籍のみ選択 | Secondary | 退出済みユーザーのみ選択 |

**Row 4 - 削除ボタン:**

| コンポーネント | emoji | ラベル | スタイル | 動作 |
| --- | --- | --- | --- | --- |
| `bump-reminder:remove-mention-users:submit` | 🗑️ | 削除する | Danger | 選択ユーザーを削除して完了メッセージに更新 |
...
- 追加対応（未実装）:
  - ロールが削除された場合、DBの `mentionRoleId` を消去する（`reset` コマンド実装時に対応）
  - ユーザーがサーバーを退出した場合、DBの `mentionUserIds` から該当ユーザーを削除する（`reset` コマンド実装時に対応）
...
| キー | ja | en |
| --- | --- | --- |
| `bump-reminder-config.description` | Bumpリマインダーの設定（サーバー管理権限が必要） | Configure bump reminder (requires Manage Server) |
| `bump-reminder-config.enable.description` | Bumpリマインダー機能を有効化 | Enable bump reminder feature |
| `bump-reminder-config.disable.description` | Bumpリマインダー機能を無効化 | Disable bump reminder feature |
| `bump-reminder-config.set-mention.description` | メンションロールを設定 | Set mention role |
| `bump-reminder-config.set-mention.role.description` | リマインダーでメンションするロール | Role to mention in reminders |
| `bump-reminder-config.remove-mention.description` | メンションロール設定を削除 | Remove mention role setting |
| `bump-reminder-config.view.description` | 現在の設定を表示 | Show current settings |
| `user-response.enable_success` | Bumpリマインダー機能を有効化しました。 | Bump reminder feature has been enabled |
| `user-response.disable_success` | Bumpリマインダー機能を無効化しました。 | Bump reminder feature has been disabled |
| `user-response.set_mention_role_success` | メンションロールを {{role}} に設定しました。 | Mention role set to {{role}} |
| `user-response.set_mention_error` | メンションロールの設定に失敗しました。 | Failed to set mention role. |
| `user-response.remove_mention_role` | メンションロールの登録を削除しました。 | Mention role registration has been removed |
| `user-response.reminder_message_disboard` | ⏰ `/bump` が出来るようになったよ！ | ⏰ `/bump` is ready! |
| `user-response.reminder_message_dissoku` | ⏰ `/up` が出来るようになったよ！ | ⏰ `/up` is ready! |
| `user-response.reminder_message` | ⏰ **Bump出来るようになったよ！** | ⏰ **Ready to bump!** |
| `user-response.panel_scheduled_at` | <t:{{timestamp}}:R>にリマインドが通知されます。 | Reminder will be sent <t:{{timestamp}}:R>. |
| `user-response.panel_mention_toggled_on` | 通知をONにしました。 | Notification turned ON. |
| `user-response.panel_mention_toggled_off` | 通知をOFFにしました。 | Notification turned OFF. |
| `user-response.panel_update_failed` | Bump通知リストの更新に失敗しました。 | Failed to update the notification list. |
| `embed.title.success` | 設定完了 | Settings Updated |
| `embed.description.not_configured` | Bumpリマインダーが設定されていません。 | Bump reminder is not configured. |
| `embed.title.config_view` | Bumpリマインダー機能 | Bump Reminder Feature |
| `embed.description.config_view` | 現在の設定状態 | Current settings status |
| `embed.title.panel` | Bumpリマインダー機能 | Bump Reminder |
| `embed.field.name.status` | 状態 | Status |
| `embed.field.name.mention_role` | メンションロール | Mention Role |
| `embed.field.name.mention_users` | メンションユーザー | Mention Users |
| `ui.button.mention_on` | ユーザー通知をONにする | Turn Notification ON |
| `ui.button.mention_off` | ユーザー通知をOFFにする | Turn Notification OFF |
| `log.detected` | Bumpを検知 GuildId: {{guildId}} Service: {{service}} | bump detected GuildId: {{guildId}} Service: {{service}} |
| `log.detection_failed` | Bump検知処理に失敗 GuildId: {{guildId}} | failed to handle bump detection GuildId: {{guildId}} |
| `log.panel_mention_updated` | メンション {{action}} UserId: {{userId}} GuildId: {{guildId}} | mention {{action}} UserId: {{userId}} GuildId: {{guildId}} |
| `log.panel_handle_failed` | パネルボタン処理失敗 | Failed to handle panel button |
| `log.panel_reply_failed` | パネルボタン エラー返信失敗 | Failed to send error reply for panel button |
| `log.config_enabled` | 有効化 GuildId: {{guildId}} ChannelId: {{channelId}} | enabled GuildId: {{guildId}} ChannelId: {{channelId}} |
| `log.config_disabled` | 無効化 GuildId: {{guildId}} | disabled GuildId: {{guildId}} |
| `log.config_mention_set` | メンションロール設定 GuildId: {{guildId}} RoleId: {{roleId}} | mention role set GuildId: {{guildId}} RoleId: {{roleId}} |
| `log.config_mention_removed` | メンション設定削除 GuildId: {{guildId}} Target: {{target}} | mention settings removed GuildId: {{guildId}} Target: {{target}} |

### 新規追加予定キー（reset / remove-mention-users）

| キー | 用途 | メッセージ（ja） | メッセージ（en） |
| --- | --- | --- | --- |
| `bump-reminder-config.reset.description` | `reset` サブコマンド説明 | Bumpリマインダー設定をリセット | Reset bump reminder settings |
| `bump-reminder-config.remove-mention-users.description` | `remove-mention-users` サブコマンド説明 | 通知ユーザーを削除 | Remove notification users |
| `user-response.reset_success` | リセット成功レスポンス | Bumpリマインダー設定をリセットしました。 | Bump reminder settings have been reset. |
| `user-response.reset_cancelled` | リセットキャンセル | リセットをキャンセルしました。 | Reset has been cancelled. |
| `user-response.remove_users_success` | ユーザー削除成功レスポンス | 選択したユーザーを通知リストから削除しました。 | Selected users have been removed from the notification list. |
...
| `ui.button.select_left` | サーバー非在籍のみ選択ボタン | サーバー非在籍のみ選択 | Select Non-Members Only |

...
メッセージ削除機能 - モデレーター向けメッセージ一括削除（**旧仕様**）。

> [!NOTE]
> **新仕様への移行中**: UIフロー（確認ダイアログの2段階化・削除前プレビュー・除外機能）を大幅に変更中。
> 新仕様は [MESSAGE_DELETE_SPEC_V2.md](specs/MESSAGE_DELETE_SPEC_V2.md) を参照。
> 移行完了後にこの旧仕様ドキュメントは削除予定。
...
#### [MESSAGE_DELETE_SPEC_V2.md](specs/MESSAGE_DELETE_SPEC_V2.md)

メッセージ削除機能 - 新仕様（**設計中・移行中**）。

**旧仕様からの主な変更点:**

- 確認ダイアログは常に表示（「次回から確認しない」廃止）
- 削除前プレスキャン＋メッセージプレビュー（フィルター・削除対象からの除外機能付き）
- 上記プレビューダイアログ（第1段）＋最終確認ダイアログ（第2段）の2段階確認
- 削除後の詳細表示を廃止し、シンプルな完了メッセージに変更
...
- `/guild-config view` — ページパネル（概要＋各機能設定）、ページネーションボタン＋セレクトメニュー

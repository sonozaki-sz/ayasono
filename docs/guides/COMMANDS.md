...
最終更新: 2026年3月13日（VC募集機能実装済みに更新・メッセージ削除 V2 対応・/message-delete-config 廃止）
...
サーバー内のメッセージを一括削除します。スキャン → プレビュー → 最終確認 → 削除の4フェーズで安全に実行できます。

**構文:**

```
/message-delete [count] [user] [bot] [keyword] [days] [after] [before] [channel]
```

**オプション:**

- `count` (オプション): 削除件数の上限（1〜1000）。未指定時は最新1000件を上限として削除
- `user` (オプション): 特定ユーザー・Bot・Webhookのメッセージのみ対象（`@メンション` または生ID）
- `bot` (オプション): Botメッセージのみ対象（`true` / `false`）
...
1. **Phase 1 スキャン** — 全チャンネルを横断してフィルター条件に一致するメッセージをスキャン（リアルタイム進捗表示）
2. **Phase 2 プレビュー** — 一致したメッセージを5件/ページで一覧表示（ページネーション付き）
3. **Phase 3 最終確認** — `⚠️ この操作は取り消せません` を確認してから実行
4. **Phase 4 削除実行** — リアルタイムで削除進捗を表示し、完了メッセージを送信
...
# 特定ユーザーのメッセージを削除
/message-delete user:@Spammer

# キーワードを含むメッセージを削除
/message-delete keyword:スパム

# 過去7日間の特定ユーザーのメッセージを削除
/message-delete user:@BadUser days:7

# 特定チャンネルのBotメッセージを削除
/message-delete bot:true channel:#bot-spam
```

**注意事項:**

- フィルター条件（`count` 以外のオプション）を1つ以上指定する必要があります（安全のため）

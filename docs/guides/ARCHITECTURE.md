...
│  ┌──────────────────┐    ┌──────────────────────────┐   │
│  │  Bot プロセス      │    │  Web プロセス             │   │
│  │  src/bot/main.ts │    │  src/web/server.ts       │   │
│  │                  │    │                          │   │
│  │  Discord Bot     │    │  Fastify HTTP Server     │   │
│  │  Gateway 接続     │    │  REST API + 静的配信      │   │
│  └────────┬─────────┘    └───────────┬──────────────┘   │
│           │                          │                  │
│           └──────────┬───────────────┘                  │
│                      ▼                                  │
...
│   │   ├── guildConfigRepositoryProvider.ts # Repository提供（内部実装）
...
- src整備フェーズでは、実装整理の完了判定を優先し、テスト修正フェーズは分離する
- 順序は **再分析 → コメント規約反映 → ドキュメント同期 → TODO同期 → テスト修正** を原則とする
- この順序により、構造変更とテスト追随を同一フェーズで混在させず、差分の責務を明確化する
...
├── commands/             # コマンド実行処理（例: `vacCommand.execute.ts`）
...
詳細な実装運用（直接import規約、公開APIの絞り方）は
[IMPLEMENTATION_GUIDELINES.md](IMPLEMENTATION_GUIDELINES.md) の「`index.ts`（バレル）禁止ルール」を参照してください。

### import 境界ルール（運用）

- 公開境界・内部実装を問わず、`index.ts` 経由 import は利用しない
- import は常に実体モジュール（役割名ファイル/実装ファイル）を直接参照する
- この方針により、循環依存リスクとモック追従コストを抑制する
...
機能設定（AfkConfig, BumpReminderConfig 等）は `GuildConfig` テーブルに **JSON 文字列**として保存します。
これはスキーマ変更なしに設定フィールドを追加できる利点があります。

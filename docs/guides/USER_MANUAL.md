...
- **🔔 通知を受け取る** — 2時間後のリマインダーで自分に通知が届きます。
- **🔕 通知を受け取らない** — 通知設定をキャンセルする場合に押します。

**💡 ヒント:** Bumpに協力したいときは「🔔 通知を受け取る」を押して通知をオンにしておきましょう。
...
**ステップ 1/2 — VC・メンションを選択する**

ボタンを押すと自分だけに見えるメッセージが表示されます。

<table border="1" cellpadding="8" width="420">
<tr><th align="left">📋 ステップ 1/2 — メンション・VCを選択してください</th></tr>
<tr><td>
<b>メンション</b><br>
<i>セレクトメニュー<br>
・なし（メンションしない）<br>
...
| 項目               | 説明                                                           |
| ------------------ | -------------------------------------------------------------- |
| メンション（任意） | 募集メッセージに付けるロールメンションを選択。「なし」も選択可 |
...
```
/vc-recruit-config remove-role role:@ゲーマー募集
```
...
| `user`     | 文字列     | 削除対象のユーザーID またはメンション（`<@ID>`）                               |
...
- `count`・`user`・`keyword`・`days`・`after`・`before` のいずれかを必ず指定してください。何も指定しない場合は実行できません

...
- **新規VC作成** — 既存VCへの参加のほか「新規VC作成」も選択可能。全員退出で自動削除
- **自動移動** — 募集投稿と同時に選択したVCへ自動で移動
...
管理者がセットアップした **`#vc募集`** チャンネルを開き、パネルのボタンを押します。
...
| 新規VC名（任意） | ❌   | 「🆕 新規VC作成」選択時のみ使用。未入力だと `あなたの表示名's Room` になります |

送信すると **`#vc募集板`** チャンネルに募集メッセージが投稿され、自動でそのVCへ移動します。

**💡 ヒント:**

- `#vc募集板` チャンネルへの直接書き込みはできません。募集は必ずパネルのボタンから作成してください。
- 「🆕 新規VC作成」を選ぶと、そのVC専用の操作パネルが自動で送られます（名前変更・人数制限など）。
- 投稿後に `#vc募集板` のスレッドに参加者との返信ができます。
- 送信時点でVCに参加していない場合、自動移動はスキップされます（メッセージのみ投稿されます）。
...
| チャンネル名 | 役割                                         |
| ------------ | -------------------------------------------- |
| `#vc募集`    | 募集ボタンが設置されるパネルチャンネル       |
| `#vc募集板`  | 募集メッセージが投稿される閲覧専用チャンネル |
...
> ⚠️ 撤去するとパネルチャンネル・投稿チャンネルが削除されます。この操作は取り消せません。
...
**Q. 送信後にVCへ自動移動されなかった。**
A. 送信時点でボイスチャンネルに参加していない場合、自動移動はスキップされます。先に任意のVCに入ってから募集を投稿してください。
...
**Q. VC募集チャンネルを撤去したい。**
A. `/vc-recruit-config teardown` を実行すると、セットアップ済みのカテゴリーの一覧から撤去対象を選択できます。確認パネルで「🗑 撤去する」を押すとパネルチャンネル・投稿チャンネルが削除されます。

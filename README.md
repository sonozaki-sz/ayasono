...
- `/message-delete` コマンドでサーバー全チャンネルを横断して一括削除
- ユーザー指定・キーワード・件数・期間（日数 / 日付範囲）・チャンネルを自由に組み合わせ
...
コマンドで専用チャンネルを自動作成し、VC参加者を募るメッセージをモーダルで投稿する機能です。

- `/vc-recruit-config setup` でパネルチャンネルと投稿チャンネルをカテゴリーに自動作成
- ボタン押下 → 2ステップモーダルで募集内容・メンション・VC を設定して投稿
- 「🆕 新規VC作成」選択時は専用VCを作成し、全員退出で自動削除
- 投稿後に募集者を対象VCへ自動移動、投稿チャンネルには返信用スレッドを自動作成
- メンション対象ロールを管理者があらかじめ制限、複数カテゴリーにセットアップ可能

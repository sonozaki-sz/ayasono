...
vi.mock("@/bot/services/botCompositionRoot", () => ({
  getBotMemberLogConfigService: () => ({
    setLeaveMessage: (...args: unknown[]) => setLeaveMessageMock(...args),
  }),
}));
...
function makeInteraction(message = "さようなら {user}！") {
  return {
    options: {
...
// handleMemberLogConfigSetLeaveMessage の権限チェック・保存・応答フローを検証
describe("bot/features/member-log/commands/memberLogConfigCommand.setLeaveMessage", () => {
  // 各テストでモック呼び出し記録をリセットし、テスト間の副作用を排除する
  beforeEach(() => {
    vi.clearAllMocks();
    setLeaveMessageMock.mockResolvedValue(undefined);
  });
  ...
  it("propagates error when guard throws", async () => {
    ...
    await expect(
      handleMemberLogConfigSetLeaveMessage(
  ...
  // service.setLeaveMessage が正しい引数で呼ばれることを確認
  it("calls service.setLeaveMessage with correct arguments", async () => {
    ensurePermissionMock.mockResolvedValue(undefined);
    const interaction = makeInteraction("バイバイ {username}");

    await handleMemberLogConfigSetLeaveMessage(interaction as never, "guild-1");

    expect(setLeaveMessageMock).toHaveBeenCalledWith(
      "guild-1",
      "バイバイ {username}",
    );
  });
  ...
  it("logs info on success", async () => {
    ...
    await handleMemberLogConfigSetLeaveMessage(
      makeInteraction() as never,
      "guild-1",
    );

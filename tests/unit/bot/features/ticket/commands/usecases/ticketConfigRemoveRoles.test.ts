// tests/unit/bot/features/ticket/commands/usecases/ticketConfigRemoveRoles.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketConfigRemoveRoles", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答を返す", async () => {
    const { handleTicketConfigRemoveRoles } = await import(
    ...
    await handleTicketConfigRemoveRoles(interaction as never, "guild-1");
  ...
  it("ロール選択メニューのcustomIdにREMOVE_ROLES_PREFIXとカテゴリIDが含まれる", async () => {
    const { handleTicketConfigRemoveRoles } = await import(
      "@/bot/features/ticket/commands/usecases/ticketConfigRemoveRoles"
    );
    ...
    expect(menuData.custom_id).toBe("ticket:remove-roles:category-1");
  });
});

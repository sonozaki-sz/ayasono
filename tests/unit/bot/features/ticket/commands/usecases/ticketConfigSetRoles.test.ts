// tests/unit/bot/features/ticket/commands/usecases/ticketConfigSetRoles.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketConfigSetRoles", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答を返す", async () => {
    const { handleTicketConfigSetRoles } = await import(
      "@/bot/features/ticket/commands/usecases/ticketConfigSetRoles"
    );

    findByGuildAndCategoryMock.mockResolvedValue(null);
    const interaction = createInteractionMock();

    await handleTicketConfigSetRoles(interaction as never, "guild-1");
  ...
  it("ロール選択メニューのcustomIdにSET_ROLES_PREFIXとカテゴリIDが含まれる", async () => {
    const { handleTicketConfigSetRoles } = await import(
      "@/bot/features/ticket/commands/usecases/ticketConfigSetRoles"
    );
    ...
    expect(menuData.custom_id).toBe("ticket:set-roles:category-1");
  });
});

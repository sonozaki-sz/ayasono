// tests/unit/bot/features/ticket/commands/usecases/ticketConfigSetMaxTickets.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketConfigSetMaxTickets", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答", async () => {
    const { handleTicketConfigSetMaxTickets } = await import(
    ...
    await handleTicketConfigSetMaxTickets(interaction as never, "guild-1");
  ...
  it("count パラメータが正しく渡されること", async () => {
    const { handleTicketConfigSetMaxTickets } = await import(
      "@/bot/features/ticket/commands/usecases/ticketConfigSetMaxTickets"
    );

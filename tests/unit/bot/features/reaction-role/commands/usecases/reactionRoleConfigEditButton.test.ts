// tests/unit/bot/features/reaction-role/commands/usecases/reactionRoleConfigEditButton.test.ts
...
const editButtonSessionSetMock = vi.fn();
vi.mock(
  "@/bot/features/reaction-role/handlers/ui/reactionRoleSetupState",
  () => ({
    reactionRoleEditButtonSessions: {
      set: editButtonSessionSetMock,
      get: vi.fn(),
      delete: vi.fn(),
    },
  }),
...
describe("bot/features/reaction-role/commands/usecases/reactionRoleConfigEditButton", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("パネルが0件の場合はエラー応答を返す", async () => {
    const { handleReactionRoleConfigEditButton } = await import(
    ...
    await handleReactionRoleConfigEditButton(interaction as never, "guild-1");
  ...
  it("パネルが1件の場合はボタン選択セレクトメニューを直接表示する", async () => {
    ...
    expect(selectMenu.custom_id).toContain("reaction-role:edit-button-select:");
  });

  it("パネルが1件の場合にセッションが正しいpanelIdとbuttonId=-1で作成される", async () => {
    const { handleReactionRoleConfigEditButton } = await import(
      "@/bot/features/reaction-role/commands/usecases/reactionRoleConfigEditButton"
    );
    ...
    expect(editButtonSessionSetMock).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        panelId: "panel-x",
        buttonId: -1,
  ...
  it("パネルが複数件の場合はパネル選択セレクトメニューを表示する", async () => {
    ...
    const replyCall = interaction.reply.mock.calls[0][0];
    const selectMenu = replyCall.components[0].components[0].data;
    expect(selectMenu.custom_id).toContain("reaction-role:edit-button-panel:");
  });

  it("複数パネルの場合はセッションのpanelIdが空文字でbuttonIdが-1で初期化される", async () => {
    const { handleReactionRoleConfigEditButton } = await import(
      "@/bot/features/reaction-role/commands/usecases/reactionRoleConfigEditButton"
    );

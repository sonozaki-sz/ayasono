// src/bot/features/ticket/commands/usecases/ticketConfigAddRoles.ts
// チケットスタッフロール追加処理
...
/**
 * ticket-config add-roles サブコマンドを処理する
 * @param interaction コマンド実行インタラクション
 * @param guildId ギルドID
 */
export async function handleTicketConfigAddRoles(
  ...
  const roleSelect = new RoleSelectMenuBuilder()
    .setCustomId(`${TICKET_CUSTOM_ID.ADD_ROLES_PREFIX}${categoryId}`)
    .setPlaceholder(
